...
class DataStream(ABC):
    ...
    def filter_data(
        ...
        return [item for item in data_batch if lowered in str(item).lower()]
    ...
    def filter_data(
        ...
        if criteria == "critical":
            return [
                ...
                if "error" in str(item).lower() or "alert" in str(item).lower()
            ]
...
def _safe_print(label: str, action: Callable[[], None]) -> None:
    try:
        print(label)
        action()
...
if __name__ == "__main__":
    ...
    filtered_sensor = sensor_stream.filter_data(
        ["alert:temp", "ok", "error:pressure"], "critical"
    )
    filtered_trans = transaction_stream.filter_data(
        ["buy:20", "sell:150"], "large"
    )
    print(
